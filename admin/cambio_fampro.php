...
<head>
    ...
    <title>ACTUALIZAR FAMILIA DE PRODUCTOS</title>
...
<body>
    ...
    $consulta = "SELECT fp.*, sbc.nom_sbcat
        FROM familia_producto fp, subcategoria sbc
        WHERE fp.id_sbcat = sbc.id_sbcat AND fp.id_fam_pro = '$id'";
    $ejecucion = $conexion->query($consulta);
    $imprimir = $ejecucion->fetch_assoc();
    ?>
    <center>
        <fieldset>
            <legend><h1>FAMILIA DE PRODUCTOS</h1></legend>
            <form action="guardar_cambio_fampro.php?id=<?php echo $imprimir['id_fam_pro']; ?>" method="POST">
                <label>NOMBRE DE FAMILIA: </label>
                <input type="text" name="familia" value="<?php echo $imprimir['nom_fam_pro'];?>" placeholder="Familia de Productos..." required>
                <br><br>
                <span>
                    ...
                    <select name="sbcat" required>
                        <option value="<?php echo $imprimir['id_sbcat']; ?>">--- <?php echo $imprimir['nom_sbcat']; ?> ---</option>
